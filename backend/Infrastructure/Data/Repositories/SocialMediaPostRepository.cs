...
internal sealed class SocialMediaPostRepository : ISocialMediaPostRepository
{
    ...
    public async Task<SocialMediaPost?> GetByIdAsync(string id)
    {
        ...
        try
        {
            return await _collection
                ...
                .FirstOrDefaultAsync(cancellationToken: CancellationToken.None).ConfigureAwait(false);
        }
        catch (Exception ex)
        {
    ...
    {
        await _collection.InsertOneAsync(post, cancellationToken: CancellationToken.None).ConfigureAwait(false);
        return post;
    }

    public async Task UpdateAsync(string id, SocialMediaPost post) =>
        await _collection
            .ReplaceOneAsync(filter: p => p.Id == id, replacement: post, cancellationToken: CancellationToken.None).ConfigureAwait(false);

    public async Task DeleteAsync(string id) =>
        await _collection
            .DeleteOneAsync(post => post.Id == id, cancellationToken: CancellationToken.None).ConfigureAwait(false);
    ...
    public async Task UpdateMetricsAsync(string id, int upvotes, int downvotes, int commentCount, int shareCount)
    {
        ...
        await _collection
            .UpdateOneAsync(post => post.Id == id, update, cancellationToken: CancellationToken.None).ConfigureAwait(false);
    }
}
