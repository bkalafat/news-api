...
describe("RelatedNews Component", () => {
  ...
  it("renders loading skeleton initially", () => {
    ...
    render(<RelatedNews category="technology" currentNewsId="999" limit={3} />);
  ...
  it("fetches and displays related news", async () => {
    ...
    render(<RelatedNews category="technology" currentNewsId="2" limit={3} />);
    ...
    await waitFor(() => {
      expect(screen.getByText(mockNewsArticles[0]?.caption ?? "")).toBeInTheDocument();
    });
  });
  ...
  it("filters out current news from results", async () => {
    ...
    render(<RelatedNews category="technology" currentNewsId={currentNewsId} limit={5} />);
  ...
  it("calls API with correct category parameter", async () => {
    ...
    render(<RelatedNews category={category} currentNewsId="1" limit={3} />);

    await waitFor(() => {
      expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining(`category=${category}`));
    });
  });
  ...
  it('renders "View All" link with correct category', async () => {
    ...
    render(<RelatedNews category="technology" currentNewsId="1" limit={3} />);
  ...
  it("returns null when no related news available", async () => {
    ...
    const { container } = render(<RelatedNews category="technology" currentNewsId="1" limit={3} />);
