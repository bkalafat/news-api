...
public class CreateNewsDtoValidator : AbstractValidator<CreateNewsDto>
{
    public CreateNewsDtoValidator()
    {
        ...
        RuleFor(x => x.SocialTags)
            .MaximumLength(500).WithMessage("Social tags must not exceed 500 characters");
        ...
        RuleFor(x => x.ExpressDate)
            .NotEmpty().WithMessage("Express date is required")
            .GreaterThan(DateTime.MinValue).WithMessage("Express date must be valid");
